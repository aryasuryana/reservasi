


<div class="footer">

<div class="container">




             <div class="row">
            <!-- <div class="col-lg-4 col-sm-4"> -->
                    <!-- <h4>Ikuti Kami</h4>
                    <a href="https://wa.me/6285822336586"><img src="images/wa.png" alt="whatsapp"></a>
                    <a href="https://www.instagram.com/lanlanaaa_/"><img src="images/instagram.png" alt="instagram"></a> -->
            </div>


           
            <div class="col-lg-4 col-sm-4">
              <!--      <h4>Informasi</h4>
                   <ul class="row">
                <li class="col-lg-12 col-sm-12 col-xs-3"><a href="index.php">Dashboard</a></li>
                <li class="col-lg-12 col-sm-12 col-xs-3"><a href="blog.php">Daftar gedung</a></li>         
                <li class="col-lg-12 col-sm-12 col-xs-3"><a href="agents.php">Tambah Gedung</a></li>
                <li class="col-lg-12 col-sm-12 col-xs-3"><a href="contact.php">Pemesanan</a></li>
              </ul> -->
            </div>

             <div class="col-lg-4 col-sm-4">
                    <h4>Kontak</h4>
                    <p><b>PT. Cinta Sejati</b><br>
<span class="glyphicon glyphicon-map-marker"></span> HBI, Banjarmasin <br>
<span class="glyphicon glyphicon-envelope"></span> emailini@gmail.com<br>
<span class="glyphicon glyphicon-earphone"></span> 085822336586</p>
            </div>
        </div>
<!-- <p class="copyright">Copyright 2022.</p> -->


</div></div>





</body>
</html>



